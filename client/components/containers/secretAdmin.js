...
class AdminPanel extends Component {
  ...
  render() {
    const usernameErr = !this.props.users.loggingIn && this.props.users.currentErrors.length > 0 && this.props.users.currentErrors[0].username;
    const passErr = !this.props.users.loggingIn && this.props.users.currentErrors.length > 0 && this.props.users.currentErrors[0].password;
    ...
    return (
      <div className="admin-client">
        <div className="loginForm">
          ...
          <h2>Admin Login</h2>
          <input type="username" name="username" className="sign-field" value={this.props.users.username} placeholder="username" onChange={this.setCredentials} disabled={this.props.users.loggingIn ? true : false} autoComplete="off" />
          {usernameErr && <p className="err-text">{usernameErr}</p>}
          <input type="password" name="password" className="sign-field" value={this.props.users.password} placeholder="password" onChange={this.setCredentials} disabled={this.props.users.loggingIn ? true : false} />
          {passErr && <p className="err-text">{passErr}</p>}
          <button className="cta-defaults" onClick={this.accountLogin} disabled={this.props.users.loggingIn ? true : false}>
            {this.props.users.loggingIn ? <span className="general-loader" /> : "Proceed"}
          </button>
